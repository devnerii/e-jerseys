        ...
        <section>
            <div class="grid grid-cols-2 gap-8 md:grid-cols-4 mb-16">
                @foreach ($produtos_principais as $produto)
                    ...
                    @endphp
                    <a href="{{ route('site.product', ['homePageSlug' => $produto->homepage->slug, 'slug' => $produto['slug']]) }}">
                        <div class="col-span-1 bg-white shadow-lg shadow-slate-100 rounded-xl">
                            ...
                            <div class="px-2 py-4 text-lg font-bold text-center text-primary">
                                ...
                                @if ($produto->price_full != null && $produto->price_full != 0)
                                    <span class="text-sm font-normal text-gray-600 line-through">{{ formatCurrency($produto->price_full) }}</span>
                                @endif
                            </div>
                        </div>
                    </a>
            ...
            <div class="text-center move-down" x-data="{ hover: false }">
                <a href="{{ route('site.products', ['homePageSlug' => $homePage->slug, 'slug' => $slug]) }}" x-on:mouseover="hover = true" x-on:mouseleave="hover = false"
    ...
    <section>
        <div class="grid grid-cols-2 gap-8">
            @foreach ($produtos_destaques as $produto)
                ...
                @endphp
                <a href="{{ route('site.product', ['slug' => $produto['slug'], 'homePageSlug' => $produto->homepage->slug]) }}">
                    <div class="col-span-1 bg-white shadow-lg shadow-slate-100 rounded-xl">
                        <div class="relative overflow-hidden">
