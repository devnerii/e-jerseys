...
class DetectBotMiddleware
{
    ...
    private function detectBot($request)
    {
        ...
        $isSuspiciousUserAgent = $this->isSuspiciousUserAgent($userAgent);

        $isSuspiciousHeaders = $this->isSuspiciousHeaders($request->headers->all());
    ...
    private function getSessionLength($request)
    {
        ...
        $sessionStart = session()->get('session_start', now()->timestamp);
        session()->put('session_start', $sessionStart);
