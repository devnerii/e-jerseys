...
{
    public function index($homePageSlug, $slug)
    {
        $produto = Product::where('slug', $slug)->first();

        if ($produto) {
            $homePage = HomePage::where('slug', $homePageSlug)->where('id', $produto->home_page_id)->firstOrFail();
        } else {
            $homePage = HomePage::where('homePageSlug', $homePageSlug)->firstOrFail();
        }
    ...
    public function all($homePageSlug, $slug, Request $request)
    {
        $homePage = HomePage::where('slug', $homePageSlug)->firstOrFail();
    ...
    public function cartAdd(Request $request)
    {
        ...
        $discount = collect($product->quantity_discounts)->filter(
            fn ($value) => $cart[$key]['quantity'] >= $value['min_quantity'] && $cart[$key]['quantity'] <= $value['max_quantity']);

        if ($discount->count() > 0) {
            $cart[$key]['discounted_price'] = $discount[0]['discounted_price'];
        } else {
            $cart[$key]['discounted_price'] = $cart[$key]['price'];
        }

        $cartTotal = 0.00;
        $cartDiscountedTotal = 0.00;

        foreach ($cart as $item) {
            $cartTotal += $item['price'] * $item['quantity'];
            $cartDiscountedTotal += $item['discounted_price'] * $item['quantity'];
        }

        session()->put('cart', $cart);
        session()->put('cart-total', $cartTotal);
        session()->put('cart-discount-total', $cartDiscountedTotal);
